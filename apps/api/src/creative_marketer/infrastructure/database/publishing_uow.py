...
from creative_marketer.publishing.application import PublishingRepository, PublishingUnitOfWork

from .audit import PostgresAuditWriter
from .event_delivery import PostgresOutboxWriter
from .publishing_repositories import SqlAlchemyPublishingRepository


class SqlAlchemyPublishingUnitOfWork:
    ...
    async def __aenter__(self) -> "SqlAlchemyPublishingUnitOfWork":
        ...
        self.publishing = SqlAlchemyPublishingRepository(self._session)
    ...
    async def commit(self) -> None:
        if self._transaction is None:
            raise RuntimeError("Publishing unit of work has not been entered")
        await self._transaction.commit()


class SqlAlchemyPublishingUnitOfWorkFactory:
    def __init__(self, factory: async_sessionmaker[AsyncSession]) -> None:
        self._factory = factory

    def __call__(self, tenant_id: UUID) -> PublishingUnitOfWork:
        return SqlAlchemyPublishingUnitOfWork(self._factory, tenant_id)
