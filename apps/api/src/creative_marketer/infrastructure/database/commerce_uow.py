...
from creative_marketer.commerce.application import CommerceRepository, CommerceUnitOfWork
...
from .commerce_repositories import SqlAlchemyCommerceRepository


class SqlAlchemyCommerceUnitOfWork:
    ...
    async def __aenter__(self) -> "SqlAlchemyCommerceUnitOfWork":
        session = self.factory()
        self.session = session
        self.transaction = await session.begin()
        await session.execute(
            text("SELECT set_config('app.current_tenant_id', :tenant_id, true)"),
            {"tenant_id": str(self.tenant_id)},
        )
        self.commerce = SqlAlchemyCommerceRepository(session)
        self.audit = PostgresAuditWriter(session)
        self.outbox = PostgresOutboxWriter(session)
        return self
...
class SqlAlchemyCommerceUnitOfWorkFactory:
    def __init__(self, factory: async_sessionmaker[AsyncSession]) -> None:
        self.factory = factory

    def __call__(self, tenant_id: UUID) -> CommerceUnitOfWork:
        return SqlAlchemyCommerceUnitOfWork(self.factory, tenant_id)
