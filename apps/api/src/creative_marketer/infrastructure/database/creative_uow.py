...
from creative_marketer.creative.application import CreativeRepository, CreativeUnitOfWork
from creative_marketer.events.application import OutboxWriter

from .audit import PostgresAuditWriter
from .creative_repositories import SqlAlchemyCreativeRepository
from .event_delivery import PostgresOutboxWriter


class SqlAlchemyCreativeUnitOfWork:
    ...
    async def __aenter__(self) -> "SqlAlchemyCreativeUnitOfWork":
        ...
        self.creative = SqlAlchemyCreativeRepository(self._session)
    ...
    async def commit(self) -> None:
        if self._transaction is None:
            raise RuntimeError("Creative unit of work has not been entered")
        await self._transaction.commit()


class SqlAlchemyCreativeUnitOfWorkFactory:
    def __init__(self, factory: async_sessionmaker[AsyncSession]) -> None:
        self._factory = factory

    def __call__(self, tenant_id: UUID) -> CreativeUnitOfWork:
        return SqlAlchemyCreativeUnitOfWork(self._factory, tenant_id)
