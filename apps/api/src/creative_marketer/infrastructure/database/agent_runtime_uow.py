...
from creative_marketer.agent_runtime.application import AgentRunRepository, AgentRuntimeUnitOfWork
...
class SqlAlchemyAgentRuntimeUnitOfWork:
    ...
    async def __aenter__(self) -> "SqlAlchemyAgentRuntimeUnitOfWork":
        ...
        self.runs = SqlAlchemyAgentRunRepository(self._session)
    ...
    async def commit(self) -> None:
        if self._transaction is None:
            raise RuntimeError("AgentRuntime unit of work has not been entered")
        await self._transaction.commit()


class SqlAlchemyAgentRuntimeUnitOfWorkFactory:
    def __init__(self, factory: async_sessionmaker[AsyncSession]) -> None:
        self._factory = factory

    def __call__(self, tenant_id: UUID) -> AgentRuntimeUnitOfWork:
        return SqlAlchemyAgentRuntimeUnitOfWork(self._factory, tenant_id)
